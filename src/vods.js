...
export default function Vods(props) {
  ...
  useEffect(() => {
    document.title = "VODS - Sleepy";
    const fetchVods = async () => {
      await fetch("https://archive.overpowered.tv/sleepy/vods?$limit=50&$sort[createdAt]=-1", {
        method: "GET",
        headers: {
  ...
  const fetchNextVods = async () => {
    ...
    let next = skip + 50;
    await fetch(
      `https://archive.overpowered.tv/sleepy/vods?$limit=50&$skip=${next}&$sort[createdAt]=-1`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
