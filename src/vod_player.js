...
export default function VodPlayer(props) {
  ...
  useEffect(() => {
    document.title = `${props.match.params.vodId} Vod - Sleepy`;
    const fetchVodData = async () => {
      fetch(
        `https://archive.overpowered.tv/sleepy/vods/${props.match.params.vodId}`,
        {
          method: "GET",
          headers: {
  ...
  useEffect(() => {
    ...
    const fetchNextComments = async (next) => {
      await fetch(
        `https://archive.overpowered.tv/sleepy/logs?vod_id=${props.match.params.vodId}&$limit=100&content_offset_seconds[$gte]=${player_offset}&$skip=${next}&$sort[content_offset_seconds]=1`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  const fetchComments = async (offset) => {
    await fetch(
      `https://archive.overpowered.tv/sleepy/logs?vod_id=${props.match.params.vodId}&$limit=100&content_offset_seconds[$gte]=${offset}&$sort[content_offset_seconds]=1`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
